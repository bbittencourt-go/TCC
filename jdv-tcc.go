...
var (
	i, j, num, ind, gd, t, l, vd, ve, vb, vc, vd1, vd2, vd3, vd4, soma int
	phi [Nx * Ny]int // rede
...
func op(num int, phi [Nx * Ny]int) {
	f := fmt.Sprintf("jdv-tcc-%v.dat", num) // arquivo jdv-0.dat (condição inicial)
...
func main() {
	...
	f, _ := os.Create("datjdv-tcc.dat") // cria o arquivo .dat de densidade populacional
	...
	for t = 1; t <= NG; t++ { // conta as gerações
		for gd = 0; gd < Nx*Ny; gd++ { // conta todas as células da rede
			for i = 0; i < Nx; i++ {
				for j = 0; j < Ny; j++ {
					...
					soma = phi[vd] + phi[ve] + phi[vb] + phi[vc] + phi[vd1] + phi[vd2] + phi[vd3] + phi[vd4] // soma dos estados
		...
		l++ // aumenta a cada geração
		op(l, phi) // cria o arquivo .dat a cada geração (jdv-1, jdv-2...)
